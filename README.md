...
xNVTEXx es un plugin moderno y sin dependencias, diseñado para optimizar la
creación y compilación de documentos LaTeX de forma nativa dentro de Neovim.
Elimina la necesidad de ejecutores de tareas externos, proporcionando un
ecosistema totalmente integrado, impulsado por Lua para administrar, compilar y
depurar flujos de trabajo LaTeX sin problemas.
...
xNVTEXx ofrece once comandos:

- **NVTexNewProject**: crea un proyecto nuevo (directorio del proyecto,
  repositorio Git y tex file con el nombre del proyecto).

- **NVTexCompile**: compila utilizando LuaLaTeX (default), pdfLaTeX o XeLaTeX;
  además, limpia los archivos auxiliares generados por la compilación.
...
- **NVTexCitePicker**: lee tu archivo refs.bib en la ruta del proyecto, abre un
  picker con las entradas disponibles. Al seleccionar una o varias entradas,
  seguira un menú para elegir el macro de cita que deseas usar.

  > ⚠️
  > Por ahora sólo es compatible con Snacks.picker
...
xNVTEXx offers eleven commands:

- **NVTexNewProject**: Create a new project (Project Board, Git repository and
  Tex File with the name of the project).

- **NVTexCompile**: compiles using LuaLaTeX (default), pdfLaTeX or XeLaTeX;
  Additionally, it cleans the auxiliary files generated by the compilation.
  - `:NVTexCompile lualatex`
  - `:NVTexCompile pdflatex`
  - `:NVTexCompile pdfxe`
  - `:NVTexCompile cleanmain`
...
- **NVTexCitePicker**: read your refs.bib file in the project path, open a
  picker with the available entries. When selecting one or more entries, a menu
  will follow to choose the appointment macro you want to use.

  > ⚠️
  > For now it is only compatible with Snacks.picker
...
El plugin te permite crear tus propias plantillas con un nombre determinado o
crear nuevos comandos de compilación, además de modificar los defaults:

The plugin allows you to create your own templates with a specific name or
create new compilation commands, in addition to modifying the defaults:
...
require("xNVTEXx").setup({
  ...
  commands = {
    my_cmd = { "pdflatex", "-draftmode", "{main_file}" },
  },
})
```

> ❗
> Asegúrate de usar el placeholder `{main_file}` para asegurar que se compile
> el archivo principal del proyecto.
>
> Make sure to use the `{main_file}` placeholder to ensure that the main
> project file is compiled.
